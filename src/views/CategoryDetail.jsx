...
const POST_TYPE_FILTERS = [
  { value: "all",          en: "All",          ta: "அனைத்தும்",        emoji: "🗂️" },
  { value: "complaint",   en: "Complaints",   ta: "புகார்கள்",        emoji: "⚠️" },
  { value: "alert",       en: "Alerts",       ta: "எச்சரிக்கைகள்",   emoji: "🛡️" },
  { value: "appreciation",en: "Appreciation", ta: "பாராட்டு",         emoji: "⭐" },
  { value: "local_update",en: "Local Updates",ta: "உள்ளூர் புதுப்பிப்பு", emoji: "📢" },
  { value: "discussion",  en: "Discussions",  ta: "விவாதங்கள்",       emoji: "💬" },
];
...
export default function CategoryDetail({ initialSlug, initialData }) {
  ...
  // Reset type filter whenever the category changes
  useEffect(() => { setTypeFilter("all"); }, [slug]);
        ...
        <div className="flex flex-wrap gap-2">
          {POST_TYPE_FILTERS.map((f) => {
            const count = f.value === "all"
              ? posts.filter(isPubliclyVisible).length
              : posts.filter(isPubliclyVisible).filter((p) => p.post_type === f.value).length;
          ...
          {typeFilter !== "all" ? (
            <>
              <p className="text-slate-500 dark:text-slate-400 mb-4">
                {T(`No ${POST_TYPE_FILTERS.find(f => f.value === typeFilter)?.en?.toLowerCase()} posts in this category yet.`,
                   `இந்த வகையில் இன்னும் ${POST_TYPE_FILTERS.find(f => f.value === typeFilter)?.ta} பதிவுகள் இல்லை.`)}
              </p>
              <button
                onClick={() => setTypeFilter("all")}
                className="px-5 py-2 bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-200 rounded-xl text-sm font-medium hover:bg-slate-300 dark:hover:bg-slate-600 transition-colors mr-2"
              >
                {T("Show all posts", "அனைத்து பதிவுகளும்")}
              </button>
            </>
          ) : (
            <>
