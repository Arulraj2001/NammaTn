/**
 * VizhiTN Department Routing Module
 * Maps issue categories to responsible government authorities in Tamil Nadu.
 * This is civic guidance only — VizhiTN is not a government portal.
 */
...
export const DEPARTMENT_ROUTES = {
  streetlight: {
    ...
    complaint_template: (post) =>
      `Dear Sir/Madam,\n\nI am writing to report a non-functioning streetlight at ${post.location_text || post.area_name || post.district_name}, ${post.district_name}.\n\nIssue: ${post.title_en}\nDescription: ${post.content_en || "Streetlight not working"}\n\nThis issue has been publicly documented on VizhiTN Civic Receipt ${post.civic_receipt_id} on ${new Date(post.created_date).toLocaleDateString("en-IN")}.\n\nKindly inspect and restore the streetlight at the earliest. Photo proof is available on VizhiTN.\n\nThank you.`,
