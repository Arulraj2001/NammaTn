...
const MEGA_GROUPS = [
  ...
  },
  {
    ...
    ta_title: "உதவி & ஆதரவு",
    items: [
      { path: "/help", icon: HelpCircle, en: "Help", ta: "உதவி", desc_en: "How to use VizhiTN", desc_ta: "VizhiTN எப்படி பயன்படுத்துவது" },
            ...
            <nav className="hidden lg:flex items-center gap-0.5 xl:gap-1 flex-1 ml-1 xl:ml-3">
              ...
              <div ref={megaRef} className="relative flex-shrink-0">
                ...
                >
                  ...
                  {T("Directory", "சேவைகள்")}
                  <ChevronDown className={`w-3 h-3 xl:w-3.5 xl:h-3.5 transition-transform ${megaOpen ? "rotate-180" : ""}`} />
                </button>
                ...
                <AnimatePresence>
                  {megaOpen && (
                    <motion.div
                      ...
                      className="absolute top-full right-0 mt-2 w-[740px] max-w-[calc(100vw-2rem)] max-h-[85vh] overflow-y-auto bg-white dark:bg-slate-900 rounded-3xl shadow-2xl border-2 border-slate-300 dark:border-slate-700 p-5 sm:p-6 z-50 scrollbar-thin"
                    >
                      <div className="grid grid-cols-2 gap-x-6 gap-y-6">
                        {MEGA_GROUPS.map((group, gi) => (
                          <div key={gi} className="space-y-2.5">
                            <p className="text-xs font-extrabold text-blue-600 dark:text-blue-400 uppercase tracking-wider px-1">
                              ...
                              {group.items
                                ...
                                return (
                                  <Link
                                    key={item.path}
                                    to={item.path}
                                    className="flex items-center gap-3 p-2.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/70 dark:bg-slate-800/60 hover:bg-blue-50/90 dark:hover:bg-blue-950/50 hover:border-blue-400 dark:hover:border-blue-500 shadow-2xs hover:shadow-sm group transition-all duration-200"
                                  >
                                    <div className="w-8 h-8 rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 flex items-center justify-center flex-shrink-0 group-hover:bg-blue-600 group-hover:border-blue-600 transition-colors shadow-2xs">
                                      <Icon className="w-4 h-4 text-blue-600 dark:text-blue-400 group-hover:text-white transition-colors" />
                                    </div>
                                    <div className="flex-1 min-w-0">
                                      <p className="text-xs font-extrabold text-slate-900 dark:text-white group-hover:text-blue-700 dark:group-hover:text-blue-400 transition-colors leading-snug">
                                        {T(item.en, item.ta)}
                                      </p>
                                      <p className="text-[11px] text-slate-500 dark:text-slate-400 leading-tight truncate font-medium mt-0.5">
                                        {T(item.desc_en, item.desc_ta)}
                                      </p>
                                    </div>
                                    <ArrowRight className="w-3.5 h-3.5 text-slate-300 group-hover:text-blue-600 flex-shrink-0 transition-colors opacity-0 group-hover:opacity-100 group-hover:translate-x-0.5" />
                                  </Link>
                                );
                              })}
                            </div>
