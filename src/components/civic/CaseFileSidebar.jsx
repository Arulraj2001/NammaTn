  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white dark:bg-slate-800 border-2 border-slate-300 dark:border-slate-700 rounded-2xl p-4">
        <h3 className="text-xs font-extrabold text-slate-700 dark:text-slate-200 uppercase tracking-wider mb-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <StatCell
            ...
            valueClass={stillNotFixedCount > 0 ? "text-red-600 dark:text-red-400" : "text-slate-600 dark:text-slate-400"}
            colSpan
          />
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 border-2 border-slate-300 dark:border-slate-700 rounded-2xl p-4">
        ...
        <div className="flex items-center gap-2 mb-2">
          <span className={`w-2.5 h-2.5 rounded-full flex-shrink-0 ${esc.dot}`} />
          <span className={`text-sm font-extrabold px-2.5 py-0.5 rounded-full ${esc.badge}`}>
        ...
        {escalation.level >= 4 && (
          <div className="mt-2 flex items-start gap-2 bg-red-50 dark:bg-red-900/20 border-2 border-red-300 dark:border-red-800 rounded-xl px-3 py-2">
        ...
        {escalation.level === 3 && (
          <div className="mt-2 flex items-start gap-2 bg-orange-50 dark:bg-orange-900/20 border-2 border-orange-300 dark:border-orange-800 rounded-xl px-3 py-2">
                ...
                <div className="flex flex-col items-center">
                  <div
                    className={`w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 ${
                      isCompleted
                        ? "bg-green-600"
                        : isCurrent
                          ? "bg-blue-600 ring-2 ring-blue-400 dark:ring-blue-600"
                  ...
                  >
                    {isCompleted ? (
                      ...
                      <CircleDot className="w-3 h-3 text-white stroke-[2.5]" />
                    ) : (
      ...
      {/* ── Author Trust ── */}
      <div className="bg-white dark:bg-slate-800 border-2 border-slate-300 dark:border-slate-700 rounded-2xl px-4 py-3">
        <div className="flex items-center justify-between">
          ...
          <span
            className={`text-xs font-bold px-2.5 py-0.5 rounded-full bg-slate-100 dark:bg-slate-700 ${trustLevel.color}`}
          >
...
function StatCell({ icon: Icon, label, value, valueClass = "text-slate-800 dark:text-slate-100", colSpan }) {
  return (
    <div
      className={`bg-slate-100/90 dark:bg-slate-700/70 border border-slate-200 dark:border-slate-600 rounded-xl px-3 py-2 ${
        colSpan ? "col-span-2" : ""
      }`}
    >
      <div className="flex items-center gap-1 mb-0.5">
        <Icon className="w-3.5 h-3.5 text-slate-500 dark:text-slate-400" />
        <span className="text-[11px] font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wide">{label}</span>
      </div>
      <p className={`text-lg font-extrabold leading-tight ${valueClass}`}>{value}</p>
    </div>
  );
}
